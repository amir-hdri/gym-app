...
const nav = [
  ...
  { href: "/manager/plans",     label: "طرح‌ها",     icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg> },
  ...
  { href: "/manager/freeze-requests", label: "تعلیق‌ها", icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><rect x="6" y="4" width="4" height="16" rx="1"/><rect x="14" y="4" width="4" height="16" rx="1"/></svg> },
    ...
    <div className="min-h-screen flex flex-col" dir="rtl">
      ...
      <header className="glass sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 py-3 border-b border-white/10 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 shadow-lg shadow-rose-950/40" style={{background:"linear-gradient(135deg,#c9184a,#ff758f)"}}>
          ...
          <div>
            ...
            <p className="text-[10px] text-white/40 mt-0.5">پورتال مدیریت باشگاه</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/member/dashboard" className="btn-glass rounded-xl px-3 py-1.5 text-xs text-white/70 hover:text-white transition-all font-semibold">پنل اعضا ←</Link>
          <button 
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Desktop sidebar */}
        <aside className="hidden lg:flex flex-col w-[210px] shrink-0 border-l border-white/[0.07] bg-white/[0.02] backdrop-blur-xl py-4 px-2.5 gap-1 overflow-y-auto">
          {nav.map((item, i) => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`anim-slide-r flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs font-bold transition-all relative overflow-hidden group
                  ${active
                    ? "text-bubblegum_pink border border-rosewood/30 shadow-md shadow-rose-950/20"
                    : "text-white/40 hover:text-white/80 hover:-translate-x-0.5"
                  }`}
                style={{
                  animationDelay: `${i * 30}ms`,
                  ...(active ? { background: "linear-gradient(90deg,rgba(201,24,74,.18),rgba(201,24,74,.06))" } : {})
                }}>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 glass border-t border-white/[0.08] flex backdrop-blur-xl px-1" style={{ paddingBottom: "max(env(safe-area-inset-bottom), 8px)" }}>
        {nav.slice(0, 5).map((item) => {
          const active = pathname === item.href || (item.href !== "/manager/dashboard" && pathname.startsWith(item.href));
          return (
            <Link key={item.href} href={item.href}
              className={`flex-1 flex flex-col items-center gap-1 py-2 text-[9px] font-bold transition-colors ${active ? "text-bubblegum_pink" : "text-white/35 hover:text-white/70"}`}>

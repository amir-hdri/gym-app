"use client";

import * as React from "react";
import { useParams } from "next/navigation";
import Link from "next/link";
import { ChevronRight, Printer, Download } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/Card";
import { Badge } from "@/components/ui/Badge";
import { Button } from "@/components/ui/Button";
import { FadeIn } from "@/components/animations/FadeIn";
import { formatCurrency, formatDateTime } from "@/lib/utils";
import { Loading, ErrorDisplay } from "@/components/ui/DataState";
import { usePayment } from "@/hooks/use-api";

const statusConfig: Record<string, { label: string; variant: "success" | "warning" | "destructive" }> = {
  completed: { label: "موفق", variant: "success" }, pending: { label: "معلق", variant: "warning" }, failed: { label: "ناموفق", variant: "destructive" },
};

export default function TransactionDetailPage() {
  const params = useParams<{ id: string }>();
  const { data, isLoading, isError } = usePayment(params.id);
  if (isLoading) return <Loading />;
  if (isError) return <ErrorDisplay />;
  const payment = data?.data;
  if (!payment) return <ErrorDisplay message="تراکنش یافت نشد" />;
  const status = statusConfig[payment.status];

  return (
    <div className="space-y-6">
      <div>
        <Button variant="ghost" size="sm" asChild className="mb-2">
          <Link href="/admin/payments"><ChevronRight className="h-4 w-4" /> بازگشت به پرداخت‌ها</Link>
        </Button>
      </div>

      <FadeIn>
        <Card glass>
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle>جزئیات تراکنش</CardTitle>
              <p className="text-sm text-muted-foreground mt-1">کد پیگیری: {payment.referenceId}</p>
            </div>
            <Badge variant={status.variant} className="text-base px-4 py-1.5">{status.label}</Badge>
          </CardHeader>
          <CardContent>
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-4">
                <div>
                  <p className="text-sm text-muted-foreground">پرداخت‌کننده</p>
                  <p className="text-lg font-semibold">{payment.user ? `${payment.user.firstName} ${payment.user.lastName}` : payment.userId}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">ایمیل</p>
                  <p className="text-lg font-semibold" dir="ltr">{payment.user?.email}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">روش پرداخت</p>
                  <p className="text-lg font-semibold">{payment.method}</p>
                </div>
              </div>
              <div className="space-y-4">
                <div>
                  <p className="text-sm text-muted-foreground">مبلغ</p>
                  <p className="text-3xl font-bold text-green-600">{formatCurrency(payment.amount)}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">تاریخ</p>
                  <p className="text-lg font-semibold">{formatDateTime(payment.paidAt || payment.createdAt)}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">توضیحات</p>
                  <p className="text-base">{payment.description}</p>
                </div>
              </div>
            </div>
            <div className="mt-6 flex justify-end gap-3 border-t pt-6">
              <Button variant="outline"><Printer className="h-4 w-4" /> چاپ رسید</Button>
              <Button variant="outline"><Download className="h-4 w-4" /> دانلود PDF</Button>
            </div>
          </CardContent>
        </Card>
      </FadeIn>
    </div>
  );
}
